...
import { createApi, listApi, updateApi, deleteApi,uploadPicApi,uploadChunkApi,uploadCheckApi,uploadMergeApi } from '/@/api/admin/thing';
...
const beforeUpload = async (file: File) => {
  ...
  try {
    ...
    if (response && response.data) {
      modal.form.coverUrl = response.data;
      console.log('上传成功');
      // 处理上传成功后的逻辑，例如更新 modal.form.imageURL
    } else {
      console.error('上传失败：未收到有效的响应');
      // 处理上传失败的逻辑
    }
...
const CHUNK_SIZE = 5 * 1024 * 1024; // 5MB，你可以根据实际需求调整分片大小
const beforeUpload1 = async (file: File) => {
  ...
  try {
    ...
    for (let i = 0; i < totalChunks; i++) {
      ...
      if(i==0){
        path = await getFileMD5(chunks[i]);
      }
      formData.append('path',path);
      const response1 = await uploadCheckApi(formData)
      ...
      if (response && response.data) {
        ...
        console.error(`分片 ${i + 1} 上传失败：未收到有效的响应`);
        // 处理上传失败的逻辑
      }
    }
    const dform=new FormData();
    dform.append("path",path);
    dform.append("size",file.size);
...
const handleOk = () => {
  myform.value
      ...
      .then(() => {
        ...
        if (modal.form.coverUrl) {
          formData.append('picurl', modal.form.coverUrl)
        }
